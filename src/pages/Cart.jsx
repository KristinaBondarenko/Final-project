...
export default function Cart() {
  const cart = useGalleryStore((s) => s.cart); // Получаем корзину из хранилища состояния.
  const remove = useGalleryStore((s) => s.removeFromCart); 
  const total = cart.reduce((s, i) => s + (i.price || 0), 0); // Рассчитываем общую сумму.
  const { t } = useTranslation();

  return (
    <div className="container py-8"> 
      <h1 className="text-2xl font-semibold mt-4 mb-4">{t("header.cart")}
        </h1> 
      <div className="space-y-3">
        {cart.map((it) => ( // Отображаем список товаров в корзине
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <Link to="/checkout" className="btn-primary"> 
          {t("Оформить заказ") || "cart.checkout"}
        </Link>
      </div>
    </div>
  );
